...
import { Users, MapPin, Settings2, Trash2, AlertTriangle, Loader2 } from "lucide-react";
import axios from "../api/axios";
import type { RoomCardProps } from "../types";
import Modal from "./Modal";
import { useToast } from "../context/ToastContext";

function RoomCard({ room, isAdmin, onRefresh }: RoomCardProps) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast({
        ...
        description: `Ruangan "${room.name}" telah dihapus secara permanen.`
      });
      ...
      showToast({
        ...
        description: error.response?.data?.message || "Terjadi kesalahan saat menghapus ruangan."
      });
      setIsModalOpen(false);
    } finally {
      setIsDeleting(false);
    ...
    <>
        ...
        footer={
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={isDeleting}
              className="px-6 py-3 bg-red-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-red-700 transition-all shadow-lg shadow-red-100 flex items-center gap-2 disabled:opacity-50"
            >
              {isDeleting ? (
                <>Menghapus... <Loader2 className="animate-spin" size={16} /></>
              ) : (
                <>Hapus Ruangan <Trash2 size={16} /></>
              )}
            </button>
          </div>
        }
      >
        <div className="flex items-center gap-4 text-gray-600">
          ...
          <div>
            <p className="font-bold text-gray-900">Apakah Anda yakin ingin menghapus ruangan ini?</p>
            <p className="text-sm">Ruangan <span className="font-bold text-red-600">"{room.name}"</span> akan dihapus dan tidak dapat digunakan.</p>
          </div>
        </div>
      </Modal>
    </>
